...
// VerifyJWTToken memverifikasi token JWT menggunakan secret key.
// - tokenString: token JWT dari client
// - secret: secret string untuk verifikasi (misalnya hasil getSecret("jwt-secret"))
func VerifyJWTToken(tokenString string, secret string) (*jwt.Token, error) {
	...
	token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) {
		// Validasi algoritma kalau mau lebih strict:
